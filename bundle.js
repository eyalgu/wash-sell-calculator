...
(() => {
  ...
  // ../core/dist/decimal.js
  var require_decimal2 = __commonJS({
    "../core/dist/decimal.js"(exports) {
      "use strict";
      var __importDefault = exports && exports.__importDefault || function(mod) {
        return mod && mod.__esModule ? mod : { "default": mod };
      };
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.ZERO = exports.Decimal = void 0;
      exports.d = d;
      ...
      function roundCents(value) {
        return value.toDecimalPlaces(2, decimal_js_1.default.ROUND_HALF_UP);
      }
  ...
  var require_errors = __commonJS({
    "../core/dist/errors.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.InsufficientSharesError = exports.LotIdentificationError = exports.ValidationError = void 0;
      ...
      exports.LotIdentificationError = LotIdentificationError;
  ...
  // ../core/dist/phases/a-normalize.js
  var require_a_normalize = __commonJS({
    "../core/dist/phases/a-normalize.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      ...
      function validateDate(value, field, rowIdx) {
        if (!ISO_DATE_RE.test(value)) {
          throw new errors_1.ValidationError(`Invalid date "${value}" for field "${field}" in row ${rowIdx}. Expected YYYY-MM-DD.`, field, rowIdx);
        }
        const d = /* @__PURE__ */ new Date(value + "T00:00:00Z");
        if (isNaN(d.getTime())) {
          throw new errors_1.ValidationError(`Invalid date "${value}" for field "${field}" in row ${rowIdx}.`, field, rowIdx);
        }
      }
      function validateDecimalString2(value, field, rowIdx) {
        try {
          const dec = new decimal_1.Decimal(value);
          if (!dec.isFinite()) {
            throw new Error("not finite");
          }
          return dec;
        } catch {
          throw new errors_1.ValidationError(`Invalid number "${value}" for field "${field}" in row ${rowIdx}.`, field, rowIdx);
        }
      }
      ...
      function normalizeRows(ticker, rows, audit) {
        if (!ticker || ticker.trim().length === 0) {
          throw new errors_1.ValidationError("Ticker must be a non-empty string.", "ticker");
        }
        if (rows.length === 0) {
          throw new errors_1.ValidationError("At least one row is required.", "rows");
        }
        const validated = rows.map((row, idx) => {
          validateDate(row.date, "date", idx);
          validateDate(row.acquiredDate, "acquiredDate", idx);
          if (!VALID_TRANSACTION_TYPES2.includes(row.transactionType)) {
            throw new errors_1.ValidationError(`Invalid transactionType "${row.transactionType}" in row ${idx}. Must be one of: ${VALID_TRANSACTION_TYPES2.join(", ")}.`, "transactionType", idx);
          }
          if (row.action !== "BUY" && row.action !== "SELL") {
            throw new errors_1.ValidationError(`Invalid action "${row.action}" in row ${idx}. Must be "BUY" or "SELL".`, "action", idx);
          }
          const shares = validateDecimalString2(row.shares, "shares", idx);
          if (shares.lte(0)) {
            throw new errors_1.ValidationError(`Shares must be positive in row ${idx}, got "${row.shares}".`, "shares", idx);
          }
          const pricePerShare = validateDecimalString2(row.pricePerShare, "pricePerShare", idx);
          if (pricePerShare.lt(0)) {
            throw new errors_1.ValidationError(`Price per share must be non-negative in row ${idx}, got "${row.pricePerShare}".`, "pricePerShare", idx);
          }
          return {
            ...row,
            _shares: shares,
  ...
  // ../core/dist/phases/c-loss-detection.js
  var require_c_loss_detection = __commonJS({
    "../core/dist/phases/c-loss-detection.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.identifyLossPortions = identifyLossPortions;
      function identifyLossPortions(salePortions, audit) {
        ...
        for (const sp of sorted) {
          audit.emit("LOSS_DETECTED", sp.saleDate, `Loss of ${sp.gainLoss.toString()} detected on sale ${sp.saleRowKey} (${sp.shares.toString()} shares from ${sp.soldFromFragmentId})`, {
  ...
  // ../core/dist/phases/d-replacement-match.js
  var require_d_replacement_match = __commonJS({
    "../core/dist/phases/d-replacement-match.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.addDays = addDays;
      exports.allocateReplacements = allocateReplacements;
      var decimal_1 = require_decimal2();
      ...
      function allocateReplacements(lossPortions, fragments, ticker, _normalizedRows, idGen, audit) {
        ...
        for (const lossPortion of lossPortions) {
          ...
          const eligible = fragments.filter((f) => {
            if (f.ticker !== ticker)
              return false;
            if (f.purchaseDateActual < windowStart)
              return false;
            if (f.purchaseDateActual > windowEnd)
              return false;
          ...
          const disallowedLossPerShare = lossPortion.basisPerShareAtSale.minus(lossPortion.salePricePerShare);
          for (const replacementFrag of eligible) {
            if (lossSharesRemaining.lte(0))
              break;
            const availableForMatch = getAvailableForReplacement(replacementFrag);
            if (availableForMatch.lte(0))
              continue;
            const sharesToMatch = decimal_1.Decimal.min(lossSharesRemaining, availableForMatch);
            const holdingDays = daysBetween(lossPortion.originalAcquiredDateForOrdering, lossPortion.saleDate);
            const match = {
            ...
            audit.emit("REPLACEMENT_MATCHED", lossPortion.saleDate, `Matched ${sharesToMatch.toString()} replacement shares from ${replacementFrag.fragmentId} for loss on ${lossPortion.saleRowKey}`, {
  ...
  // ../core/dist/phases/e-basis-adjust.js
  var require_e_basis_adjust = __commonJS({
    "../core/dist/phases/e-basis-adjust.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.applyAdjustments = applyAdjustments;
      var decimal_1 = require_decimal2();
      var d_replacement_match_1 = require_d_replacement_match();
      function applyAdjustments(matches, fragments, idGen, audit) {
        ...
        for (const [fragmentId, fragMatches] of matchesByFragment) {
          ...
          const totalMatched = fragMatches.reduce((sum, m) => sum.plus(m.matchedShares), decimal_1.ZERO);
          ...
          if (!hasMultipleDifferentAdjustments && fragMatches.length === 1 && totalMatched.lt(frag.sharesOpen)) {
            ...
            if (match.disallowedLossPerShare.lte(0))
              continue;
            ...
            const remainder = {
              ...
              consumedAsReplacement: decimal_1.ZERO
            };
            fragments.push(remainder);
            audit.emit("LOT_SPLIT", frag.purchaseDateActual, `Split ${frag.fragmentId}: ${totalMatched.toString()} shares adjusted, ${unadjustedShares.toString()} shares unadjusted \u2192 ${remainder.fragmentId}`, {
            ...
            frag.acquisitionDateAdjusted = (0, d_replacement_match_1.addDays)(frag.purchaseDateActual, -match.holdingPeriodDaysCarried);
            ...
            audit.emit("BASIS_ADJUSTED", frag.purchaseDateActual, `Adjusted basis of ${frag.fragmentId} by +${match.disallowedLossPerShare.toString()}/share \u2192 ${frag.basisPerShareAdjusted.toString()}/share`, {
              lotFragmentId: frag.fragmentId,
              payload: {
            ...
            audit.emit("ACQ_DATE_ADJUSTED", frag.purchaseDateActual, `Adjusted acquisition date of ${frag.fragmentId} to ${frag.acquisitionDateAdjusted} (carried ${match.holdingPeriodDaysCarried} days)`, {
              lotFragmentId: frag.fragmentId,
              payload: {
            ...
            for (const match of fragMatches) {
              ...
              if (sharesToTake.lt(workingFrag.sharesOpen)) {
                const adjustedFrag = {
                  ...
                  basisPerShareAdjusted: workingFrag.basisPerShareAdjusted.plus(match.disallowedLossPerShare),
                ...
                adjustedFrag.acquisitionDateAdjusted = (0, d_replacement_match_1.addDays)(workingFrag.purchaseDateActual, -match.holdingPeriodDaysCarried);
                fragments.push(adjustedFrag);
                audit.emit("LOT_SPLIT", workingFrag.purchaseDateActual, `Split ${workingFrag.fragmentId}: ${sharesToTake.toString()} shares adjusted (\u2192 ${adjustedFrag.fragmentId}), ${workingFrag.sharesOpen.minus(sharesToTake).toString()} remainder`, {
                ...
                audit.emit("BASIS_ADJUSTED", workingFrag.purchaseDateActual, `Adjusted basis of ${adjustedFrag.fragmentId} by +${match.disallowedLossPerShare.toString()}/share \u2192 ${adjustedFrag.basisPerShareAdjusted.toString()}/share`, {
                ...
                audit.emit("ACQ_DATE_ADJUSTED", workingFrag.purchaseDateActual, `Adjusted acquisition date of ${adjustedFrag.fragmentId} to ${adjustedFrag.acquisitionDateAdjusted} (carried ${match.holdingPeriodDaysCarried} days)`, {
                  lotFragmentId: adjustedFrag.fragmentId,
                  payload: {
                ...
                workingFrag.basisPerShareAdjusted = workingFrag.basisPerShareAdjusted.plus(match.disallowedLossPerShare);
                workingFrag.acquisitionDateAdjusted = (0, d_replacement_match_1.addDays)(workingFrag.purchaseDateActual, -match.holdingPeriodDaysCarried);
                workingFrag.washAdjustmentHistory = [
                  ...workingFrag.washAdjustmentHistory,
                  {
                    matchId: match.matchId,
                ...
                audit.emit("BASIS_ADJUSTED", workingFrag.purchaseDateActual, `Adjusted basis of ${workingFrag.fragmentId} by +${match.disallowedLossPerShare.toString()}/share \u2192 ${workingFrag.basisPerShareAdjusted.toString()}/share`, {
                  lotFragmentId: workingFrag.fragmentId,
                  payload: {
                    disallowedLossPerShare: match.disallowedLossPerShare.toString(),
                ...
                audit.emit("ACQ_DATE_ADJUSTED", workingFrag.purchaseDateActual, `Adjusted acquisition date of ${workingFrag.fragmentId} to ${workingFrag.acquisitionDateAdjusted} (carried ${match.holdingPeriodDaysCarried} days)`, {
                  lotFragmentId: workingFrag.fragmentId,
                  payload: {
                    originalAcqDate: workingFrag.purchaseDateActual,
  ...
  // ../core/dist/phases/g-output.js
  var require_g_output = __commonJS({
    "../core/dist/phases/g-output.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.buildForm8949 = buildForm8949;
      exports.buildRemainingPositions = buildRemainingPositions;
      ...
      function buildForm8949(ticker, salePortions, matches, fragments, audit) {
        ...
        const salesWithAdj = salePortions.map((portion) => ({
          portion,
          disallowedTotal: disallowedBySale.get(portion.salePortionId) ?? decimal_1.ZERO,
        ...
        for (const sale of salesWithAdj) {
          ...
          const proceeds = (0, decimal_1.roundCents)(portion.proceeds);
          const costBasis = (0, decimal_1.roundCents)(portion.shares.mul(portion.basisPerShareAtSale));
          const hasWashSale = disallowedTotal.gt(0);
          const gainOrLoss = hasWashSale ? (0, decimal_1.roundCents)(proceeds.minus(costBasis).plus(disallowedTotal)) : (0, decimal_1.roundCents)(proceeds.minus(costBasis));
          const row = {
            ...
            costBasis,
            adjustmentCode: hasWashSale ? "W" : void 0,
            adjustmentAmount: hasWashSale ? (0, decimal_1.roundCents)(disallowedTotal) : void 0,
            gainOrLoss,
            term
          };
          if (term === "SHORT") {
      ...
      function buildSummary(salePortions, matches, remainingPositions) {
        ...
        let realizedST = decimal_1.ZERO;
        let realizedLT = decimal_1.ZERO;
        let totalDisallowed = decimal_1.ZERO;
        let totalAllowed = decimal_1.ZERO;
        for (const portion of salePortions) {
          const disallowed = disallowedBySale.get(portion.salePortionId) ?? decimal_1.ZERO;
        ...
        let deferredInRemaining = decimal_1.ZERO;
        ...
        return {
          realizedGainLossShortTerm: (0, decimal_1.roundCents)(realizedST),
          realizedGainLossLongTerm: (0, decimal_1.roundCents)(realizedLT),
          totalDisallowedLosses: (0, decimal_1.roundCents)(totalDisallowed),
          deferredLossesInRemainingHoldings: (0, decimal_1.roundCents)(deferredInRemaining),
          totalAllowedLosses: (0, decimal_1.roundCents)(totalAllowed)
        };
      }
      function collectWarnings(salePortions, matches, fragments, normalizedRows) {
        const warnings = [];
        if (normalizedRows && normalizedRows.length > 0) {
          const totalBought = normalizedRows.filter((r) => r.action === "BUY").reduce((sum, r) => sum.plus(r.shares), decimal_1.ZERO);
          const totalSold = salePortions.reduce((sum, p) => sum.plus(p.shares), decimal_1.ZERO);
          const totalRemaining = fragments.reduce((sum, f) => sum.plus(f.sharesOpen), decimal_1.ZERO);
  ...
  // ../core/dist/calculator.js
  var require_calculator = __commonJS({
    "../core/dist/calculator.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      ...
      var errors_1 = require_errors();
      var a_normalize_1 = require_a_normalize();
      var c_loss_detection_1 = require_c_loss_detection();
      var d_replacement_match_1 = require_d_replacement_match();
      var e_basis_adjust_1 = require_e_basis_adjust();
      var g_output_1 = require_g_output();
      ...
      function findAndSortMatchingFragments(fragments, row) {
        ...
        if (byAcqDate.length > 0)
          return sortByHifo(byAcqDate);
      ...
      function calculate(ticker, rows, idGen) {
        const audit = new audit_1.AuditLog(idGen);
        const normalizedRows = (0, a_normalize_1.normalizeRows)(ticker, rows, audit);
        const fragments = [];
        ...
        for (const date of sortedDates) {
          ...
          for (const row of dateSells) {
            ...
            if (matched.length === 0) {
              throw new errors_1.LotIdentificationError(`Cannot identify source lots for sale ${row.rowKey}: no matching open lots found.`, row.rowKey);
            }
            let sharesToSell = row.shares;
            const totalAvailable = matched.reduce((sum, f) => sum.plus(f.sharesOpen), decimal_1.ZERO);
            if (totalAvailable.lt(sharesToSell)) {
              throw new errors_1.InsufficientSharesError(`Insufficient shares for sale ${row.rowKey}: need ${sharesToSell.toString()}, have ${totalAvailable.toString()}.`, row.rowKey, sharesToSell.toString(), totalAvailable.toString());
            }
            for (const frag of matched) {
              if (sharesToSell.lte(0))
                break;
              const sharesToTake = decimal_1.Decimal.min(frag.sharesOpen, sharesToSell);
              ...
              if (sharesToTake.lt(frag.sharesOpen)) {
                audit.emit("LOT_SPLIT", row.date, `Split ${frag.fragmentId}: sold ${sharesToTake.toString()} of ${frag.sharesOpen.toString()} shares`, {
                  lotFragmentId: frag.fragmentId,
                  saleRowKey: row.rowKey,
                  payload: {
              ...
              audit.emit("SALE_PROCESSED", row.date, `Processed sale of ${sharesToTake.toString()} shares from ${frag.fragmentId}`, {
                saleRowKey: row.rowKey,
                lotFragmentId: frag.fragmentId,
                payload: {
          ...
          const lossPortions = (0, c_loss_detection_1.identifyLossPortions)(dateSalePortions, audit);
          if (lossPortions.length > 0) {
            const dateMatches = (0, d_replacement_match_1.allocateReplacements)(lossPortions, fragments, ticker, normalizedRows, idGen, audit);
            if (dateMatches.length > 0) {
              allMatches.push(...dateMatches);
              (0, e_basis_adjust_1.applyAdjustments)(dateMatches, fragments, idGen, audit);
            }
          }
        }
  ...
  // ../core/dist/id-generator.js
  var require_id_generator = __commonJS({
    "../core/dist/id-generator.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
  ...
  // ../core/dist/builder.js
  var require_builder = __commonJS({
    "../core/dist/builder.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.AdjustedCostBasisCalculator = void 0;
      var calculator_1 = require_calculator();
      ...
      var CalculatorBuilderImpl = class {
        ...
        calculate() {
          ...
          const idGen = new id_generator_1.SequentialIdGenerator();
          const result = (0, calculator_1.calculate)(this.ticker, this.rows, idGen);
          return deepFreeze(result);
        }
      };
      var AdjustedCostBasisCalculator2 = class {
        static forTicker(ticker) {
          return new CalculatorBuilderImpl(ticker);
        }
      };
      exports.AdjustedCostBasisCalculator = AdjustedCostBasisCalculator2;
    }
  });
  ...
  var require_dist = __commonJS({
    "../core/dist/index.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.SequentialIdGenerator = exports.ZERO = exports.roundCents = exports.d = exports.Decimal = exports.InsufficientSharesError = exports.LotIdentificationError = exports.ValidationError = exports.AdjustedCostBasisCalculator = void 0;
      var builder_1 = require_builder();
      Object.defineProperty(exports, "AdjustedCostBasisCalculator", { enumerable: true, get: function() {
        return builder_1.AdjustedCostBasisCalculator;
      } });
      var errors_1 = require_errors();
      Object.defineProperty(exports, "ValidationError", { enumerable: true, get: function() {
        return errors_1.ValidationError;
      } });
      Object.defineProperty(exports, "LotIdentificationError", { enumerable: true, get: function() {
        return errors_1.LotIdentificationError;
      } });
      Object.defineProperty(exports, "InsufficientSharesError", { enumerable: true, get: function() {
        return errors_1.InsufficientSharesError;
      } });
      var decimal_1 = require_decimal2();
      ...
      Object.defineProperty(exports, "roundCents", { enumerable: true, get: function() {
        return decimal_1.roundCents;
      } });
      Object.defineProperty(exports, "ZERO", { enumerable: true, get: function() {
        return decimal_1.ZERO;
      } });
      var id_generator_1 = require_id_generator();
      Object.defineProperty(exports, "SequentialIdGenerator", { enumerable: true, get: function() {
        return id_generator_1.SequentialIdGenerator;
      } });
    }
  });
  ...
  function validateDecimalString(val, field, rowNum) {
    const trimmed = val.trim();
    if (trimmed === "") {
  ...
  function validateTransactionType(val, rowNum) {
    ...
    if (!VALID_TRANSACTION_TYPES.includes(trimmed)) {
      throw new Error(
  ...
  btnCalculate.addEventListener("click", () => {
    ...
    try {
      ...
      const result = import_core.AdjustedCostBasisCalculator.forTicker(ticker).addRows(rows).calculate();
